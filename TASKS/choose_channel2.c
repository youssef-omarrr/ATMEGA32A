...
int main() {
    ...
    while (1) {        
        ...
        if (x == '1'){
            ...
            ADC_select_ch(0);
            _delay_ms(50);
            ADC_startConv(); //when the ISR finishes the LCD gets overwritten data from before the interrupt 
            _delay_ms(50);
            initLCD4(); //so we re initialize the LCD to reset the enable and data
            _delay_ms(50);
